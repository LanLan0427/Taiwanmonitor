...
Taiwan Monitor 以台灣即時公共資訊監測為核心，聚焦以下**台灣專屬功能**：
...
| 功能 | 資料來源 | 說明 |
|------|----------|------|
| **機場航班** | 交通部 TDX 平台 | 桃園、松山、高雄機場即時到離站航班，支援機場與到離站篩選 |
| **國道路況** | 交通部 TDX 平台 | 高速公路即時車流速度、旅行時間與壅塞等級 |
...
- **UI 面板**：`src/components/Taiwan*.ts` — 六個台灣專屬面板元件
...
| [交通部 TDX 平台](https://tdx.transportdata.tw/) | 台鐵即時動態、車站資訊、機場航班、國道路況 | 依 TDX 授權規範 |
...
│   │   ├── TaiwanFlightPanel.ts     # 機場航班面板
│   │   ├── TaiwanHighwayPanel.ts    # 國道路況面板
│   │   └── layers/
...
- **[交通部 TDX 平台](https://tdx.transportdata.tw/)** — 台鐵即時動態、機場航班、國道路況
